...
Definition lap_exist (f:R->R)(M:R)(c1:R)(s:C): Prop :=
let (c,w) := s in
...
(*the following are some important Axioms*)
Axiom ex_cos_precondition: forall(f:R->R)(g:R->R)(M:R)(c1:R)(s:C),
let (c,w) := s in
lap_exist f M c1 s ->
ex_lim (fun b => RInt (fun t => ((f t) * (exp (-c * t)) * (cos (w * t)))%R ) 0 b) p_infty.

Axiom ex_sin_precondition: forall(f:R->R)(g:R->R)(M:R)(c1:R)(s:C),
let (c,w) := s in
lap_precondition f g M c1 s ->
ex_lim (fun b => RInt (fun t => ((- f t) * (exp (-c * t)) * (sin (w * t)))%R ) 0 b) p_infty /\
ex_lim (fun b => RInt (fun t => ((- g t) * (exp (-c * t)) * (sin (w * t)))%R ) 0 b) p_infty.

Axiom ex_cos_plus_precondition: forall(f:R->R)(g:R->R)(M:R)(c1:R)(s:C),
let (c,w) := s in
lap_precondition f g M c1 s ->
...
Axiom ex_sin_plus_precondition: forall(f:R->R)(g:R->R)(M:R)(c1:R)(s:C),
let (c,w) := s in
lap_precondition f g M c1 s ->
...
Axiom ex_minus_precondition: forall (f:R->R)(g:R->R)(M:R)(c1:R)(s:C),
let (c,w) := s in
lap_precondition f g M c1 s ->
...
Lemma const_lap : forall (f:R->R)(M:R)(c1:R)(s:C)(m:R),
 lap_exist f M c1 s ->
 laplace2 (m *c f) s = C_CRbar_mul m (laplace2 f s).
intros.
simpl.
unfold laplace2.
f_equal.
