...
(*
Definition rbarfconst (c:R) (cbarf:CBarF) : CBarF :=
  let (u,v) := cbarf in
    (fun t => Rbar_mult (Finite c) (u t), 
     fun t => Rbar_mult (Finite c) (v t)).
*)
...
Axiom fun_eq1 : forall (u v :C->Rbar), 
   (forall t, u t = v t) -> u = v.


(* Laplace (c * f) = c * Laplace(f) *)
Lemma const_lap : forall (c:R) (f:R->R), 
   laplace (c *c f) = C_CBarF_mul c (laplace f).
intros.
simpl.
unfold laplace.
f_equal.
...
Axiom e1: forall (f:R->R)(a b : R), Riemann_integrable f a b.
Axiom e2: forall (f:R->R) (a:Rbar), ex_lim f a.
Axiom e3: forall (f g:R->R) (a:Rbar), ex_lim f a -> ex_lim g a ->
  ex_Rbar_plus (Lim f a) (Lim g a).
Axiom e4: forall (f g:R->R)(a:Rbar), ex_lim f a -> ex_lim g a -> 
  ex_Rbar_minus (Lim f a) (Lim g a).

(*Laplace (f + g) = (Laplace f) + (Laplace g)*)
Lemma lap_plus: forall(u:R->R) (v:R->R),
  laplace (u +f v) = CBarF_plus (laplace u) (laplace v).
intros.
simpl.
unfold laplace.
f_equal.
...
Axiom Rbar_minus_pos_eq :forall(x:Rbar)(y:Rbar)(z:Rbar),
...
Axiom Rbar_minus_0_r: forall (a:Rbar),
Rbar_minus (Finite 0) a = Rbar_opp a.
Axiom Rbar_minus_0_l: forall (a:Rbar),
